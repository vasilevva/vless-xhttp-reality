...
generate_config() {                                         # Объявление функции генерации конфига
  ...
  mkdir -p data/{3x-ui,cert,logs,portainer}               # Создание всех необходимых папок
  ...
  if [ -z "$REALITY_PRIVATE_KEY" ]; then                  # Если приватный ключ не задан
    ...
    # Извлекаем приватный ключ (формат одинаковый во всех версиях)
  ...
  # Генерация пароля для Portainer если не задан
  ...
  if [ -z "$PORTAINER_HASHED_PASSWORD" ]; then            # Если хэш не создан
    ...
    PORTAINER_HASHED_PASSWORD=$(create_portainer_password_hash "$PORTAINER_PASSWORD") # Хэширование
...
setup_ssh_keys() {                                          # Объявление функции настройки SSH
  ...
  # Если мы дошли до сюда, значит ключи есть или пользователь подтвердил
...
show_info() {                                               # Объявление функции вывода информации
  ...
  echo "  URL: http://localhost:${PORTAINER_PORT}"        # Локальный URL
  echo "  Логин: ${PORTAINER_USERNAME}"                   # Имя пользователя
  echo "  Пароль: ${PORTAINER_PASSWORD}"                  # Пароль
...
main() {                                                    # Объявление главной функции
  log_info "============================================================" # Заголовок
  log_info "🚀 НАЧАЛО УСТАНОВКИ VLESS + REALITY"          # Информационное сообщение
  log_info "============================================================" # Разделитель
  echo ""                                                 # Пустая строка
  
  check_requirements                                      # Шаг 1: Проверка требований
  generate_config                                         # Шаг 2: Генерация конфигурации
  setup_ip_forwarding                                     # Шаг 3: Включение IP-форвардинга
